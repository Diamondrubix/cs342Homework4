	...
	public Game(Scanner input) {
		...
		while(tryagain) {
			...
			if (typeOfGame.equals("0") || typeOfGame.equals("1")) {
						...
						if (newver) {
							...
							switch (Integer.parseInt(words[0])) {
								case 0:
									...
									break;
								case 1:
									p = new LavaRoom(input);
									break;
								case 2:
									p = new UnderwaterRoom(input);
									break;
								case 3:
									p = new DarkRoom(input);
									break;
								case 4:
									p = new SpaceRoom(input);
									break;
								case 5:
									p = new FountainRoom(input);
									break;
								case 6:
									p = new RiverRoom(input);
									break;
								default:
									p = new Place(input);
									break;
				...
				//Client cl = null;
				try {
					Client cl = Network.getClient(typeOfGame, 3001);
					name = "doesn't matter";
					...
					System.out.println("press 0 to play single player. press 1 to host multiplayer or enter an ip to spectate a host");
					typeOfGame = sc.nextLine();
					tryagain = true;
				}
